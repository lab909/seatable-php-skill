...
// File column values are arrays of {name, url} objects.
// The 'url' is: parent_path + '/' + relative_path + '/' + filename
...
$request = new UpdateRows([
    ...
    'updates'    => [[
        'row_id' => $rowId,
        'row'    => [
            'Attachment' => [[       // ← always an array, even for a single file
                'name' => $fileName,
                'url'  => $storedPath,
            ]],
        ],
...
- File column values in rows are **arrays of objects** — always wrap in `[]` even for a single file
